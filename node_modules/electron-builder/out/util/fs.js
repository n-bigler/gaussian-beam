    ...
    return walk(src, filter, (() => {
        var _ref4 = (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* (file, stat, parent) {
            if (stat.isSymbolicLink()) {
                yield (0, (_fsExtraP || _load_fsExtraP()).symlink)((yield (0, (_fsExtraP || _load_fsExtraP()).readlink)(file)), file.replace(src, destination));
                return;
            }
            if (!stat.isFile()) {
                return;
            ...
            yield fileCopier.copy(file, file.replace(src, destination), stat);
        });
